...
distance = sqrtm((n+lambda)*sigma);

sigma_points(:, 1) = mu;
sigma_points(:, 2:n+1) = bsxfun(@plus, mu, distance(:, 1:n));
sigma_points(:, n+2:2*n+1) = bsxfun(@minus, mu, distance(:, n:-1:1));
...
other_weight = 1 / (2 * (n + lambda));
w_m(2:2*n+1) = bsxfun(@plus, other_weight, w_m(2:2*n+1));
w_c(2:2*n+1) = bsxfun(@plus, other_weight, w_c(2:2*n+1));
