...
x_weighted_sum = cos(sigma_points(3, :)) * wm.';
y_weighted_sum = sin(sigma_points(3, :)) * wm.';

mu = sigma_points * wm.';
mu(3) = atan2(y_weighted_sum, x_weighted_sum);

% TODO: Recover sigma. Again, normalize the angular difference
prediction_diff = bsxfun(@minus, sigma_points, mu);
prediction_diff(3, :) = normalize_angle(prediction_diff(3, :));

sigma = wc .* prediction_diff * prediction_diff.';
